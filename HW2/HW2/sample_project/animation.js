...
var texture_filenames_to_load = [ 
     ...
     "custom_textures/skybox-bottom.jpg",
     "custom_textures/skybox-back.jpg"];
...
function Animation()    // A class.  An example of a displayable object that our class GL_Context can manage.
{
    ...
    //imported custom objects
     ...
     self.m_moon         = new Shape_From_File( "custom_objects/moon.obj", scale( 1, 1, 1 ) );
...
Animation.prototype.drawSkyBox = function(){
     ...
     var bottom = new Material( Color( 0, 0, 0, 1 ), 1,  0, 0, 1, "custom_textures/skybox-bottom.jpg" );  
     var top = new Material( Color( 0, 0, 0, 1 ), 1,  0, 0, 1, "custom_textures/skybox-top.jpg" );  
...
Animation.prototype.drawStellarObjects = function(model_transform){
    ...
    var silver = new Material( Color( .1, .1, .1, 1 ), 1,  1, 1, 40, "custom_textures/steel.jpg" );
     ...
     model_transform = mult(model_transform, translation(45, 10+Math.sin(this.graphicsState.animation_time/3000)*20, 0));
     model_transform = mult(model_transform,  rotation( this.graphicsState.animation_time/10, 1, 0,  0 ));

     this.m_sun.draw( this.graphicsState, mult(model_transform, scale(10, 10, 10)), copper);
     ...
     model_transform = mult(model_transform, rotation( this.graphicsState.animation_time/40 + 180, 0, 1,  0 ));
     model_transform = mult(model_transform, translation(45, 20 + Math.sin(this.graphicsState.animation_time/3000)*5, 0));
     model_transform = mult(model_transform,  rotation( this.graphicsState.animation_time/10, 1, 1,  1 ));

     this.m_moon.draw( this.graphicsState, mult(model_transform, scale(7, 7, 7)), silver);
